...
export async function Studentsvsboxes(req, res) {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "All sheet and worksheet IDs are required" });
  }
  ...
  try {
    ...
    const attendanceSheetDoc =
      attendanceDoc.sheetsByTitle[
        Object.keys(attendanceDoc.sheetsByTitle).find(
          (title) => title.toLowerCase() === attendanceWorkSheetLower
        )
      ];
    ...
    const quotationDoc = new GoogleSpreadsheet(
      quotationSheet,
      serviceAccountAuth
    );
    ...
    const quotationSheetDoc =
      quotationDoc.sheetsByTitle[
        Object.keys(quotationDoc.sheetsByTitle).find(
          (title) => title.toLowerCase() === quotationWorkSheetLower
        )
      ];
    ...
    const cleanedQuotationData = quotationData
      ...
      .map((quotation) => {
        ...
        console.log(
          ...
          date,
          attendanceCountByDate[date]
        );
        return {
          Date: date,
          NoOfBoxes: noOfBoxes
            ? parseFloat(noOfBoxes.replace(/,/g, "")) || 0
            : 0,
    ...
    const resultData = cleanedQuotationData
      .filter(
        (quotation) => quotation.NoOfPresents > 0 && quotation.NoOfBoxes > 0
      )
      .map((quotation) => ({
        Date: quotation.Date,
